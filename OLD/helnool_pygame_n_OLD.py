...
def monster(screen, monx, mony, posx, posy, mon_index, chemin, path, oldpx, oldpy, monsterspeed, anim_num, spr_list):
    global ganiou_play
    global ganiouproche_play
        ...
        for col in range(12):
            for lin in range(8):
                if chiffreTexture[col][lin+(digit*DIGIT_SIZE)] == 201:
                    continue
                writepixel(screen, digitColor, col+DIGIT_POSY, lin+FIRST_DIGIT_POS+(8*i))
    writepixel(screen, BLACK, DIGIT_POSY+4, FIRST_DIGIT_POS+8)
    writepixel(screen, BLACK, DIGIT_POSY+9, FIRST_DIGIT_POS+8)


def timerUpdate(screen, timerValue):
...
def safeLevelUpdate(screen, levelMap, playerPosX, playerPosY, playerAngle, sprintLevel, maxSprintLevel, spriteList):
    ...
    while run == True:
        ...
        playerPosX, playerPosY, playerAngle, sprintLevel, maxSprintLevel = player(screen, levelMap, playerPosX, playerPosY, playerAngle, sprintLevel, maxSprintLevel)
...
def levelUpdate(screen, levelMap, playerPosX, playerPosY, playerAngle, sprintLevel, maxSprintLevel, spriteList, timerValue, keyNumber, keyPool, monsterActivate, monsterPosX, monsterPosY, monsterSpeed, pathMap):
    global dt
    ...
    while run == True:
        ...
        if monsterActivate == True:
            ...
            monster(screen, monsterPosX, monsterPosY, playerPosX, playerPosY, monsterIndex, chemin, pathMap, oldPlayerPosX, oldPlayerPosY, monsterSpeed, animNum, spriteList)
            if endGame == True:
...
def level(screen, levelFileName):
    ...
    if mapFile["monstre_spawn_time"] >= 0:
        return levelUpdate(screen, levelMap, playerPosX, playerPosY, playerAngle, sprintLevel, maxSprintLevel, spriteList, timerValue, keyNumber, mapFile["key_pool"], monsterActivate, monsterPosX, monsterPosY, monsterSpeed, pathMap)
    else:
